...
/*
Ejemplo de pruebas y optimizacion con System.nanoTime():
En las primeras versiones de este codigo, el metodo esAlfaNumerico (linea 333),
usado en el progama para verificar si un nombre de usuario es alfanumerico (es decir, que esta
compuesto solo por numeros y letras), usaba un for-loop para iterar sobre el nombre y extraer cada letra
como un substring y luego verificaba si el Substring estaba contenido en el string que contenia todos los numeros validos
o al string que contenia todas las letras validas.
Mas adelante reemplace ese metodo por uno que itera sobre cada caracter (char) del nombre de usuario  y
verifica si es alfanumerico usando el metodo isLetterOrDigit.
Compare los tiempos de ejecucion (en nanosegundos) de ambas versiones del metodo para Strings de diferente largo usando nanoTime,
para asegurarme que la version del metodo que deje en el programa es efectivamente la mas eficiente. 
Escribo a continuacion los resultados del metodo esAlfaNumerico original (menos eficiente):
- String de largo 4 ("nico"): 185600 nanosegundos
- String de largo 12 ("JuanPerez123"): 146600 nanosegundos
- String de largo 20 ("MaximoDecimoMeridio"): 161600 nanosegundos
Escribo a continuacion los resultados del metodo esAlfaNumerico que use finalmente (mas eficiente):
- String de largo 4 ("nico"): 120000 nanosegundos
- String de largo 12 ("JuanPerez123"): 15500 nanosegundos
- String de largo 20 ("MaximoDecimoMeridio"): 72000 nanosegundos
Se evidencia que la version final del metodo esAlfaNumerico tiene menor tiempo de ejecucion. De todas formas, intente recrear la
version original del metodo y la deje comentada en el codigo.
*/
...
public class Exp3_S8_Nicolas_Carrizo {
    ...
    */
    static boolean esAlfaNumerico(String texto){
        ...
        //System.out.println("Tiempo de ejecucion de esAlfaNumerico: " + duracion + " nanosegundos. Largo del texto: " + texto.length());
        return true;
    }
    //Metodo esAlfaNumerico original (menos eficiente)
    /*
    static boolean esAlfaNumerico(String texto){
        //long inicio = System.nanoTime();
        String numerosValidos = "1234567890";
        String letrasValidas = "qwertyuiopasdfghjklzxcvbnm- ";
        if (texto.length() == 0){
            return false;
        } else {
            for (int i = 0; i < texto.length(); i++){
                String letra = texto.substring(i, i+1).toLowerCase();
                if (!numerosValidos.contains(letra) && !letrasValidas.contains(letra)){
                    return false;
                }
            }
        }
